...
?>
 ...
 <?=menu("Nouilles",$nbreprod,$pseudo,"produits")?>
 <?php
$sql="SELECT * FROM produits WHERE catp='nouille' and qtep<>0 and (statutprod is null or statutprod<>'supprime')  ";
...
$sql="SELECT * FROM produits WHERE catp='nouille' and qtep<>0 and (statutprod is null or statutprod<>'supprime') LIMIT ".$index_limit.','.$result_par_page;
$result=$con->query($sql);
...
for ($page=1;$page<=$nombre_page;$page++){
  echo '<a href=nouille_client.php?page='.$page.'>'.$page.'</a> ';
}
?>
</div>
