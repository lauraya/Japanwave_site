...
?>
 ...
 <?=menu("Pâtisseries",$nbreprod,$pseudo,"produits")?>
 <?php
$sql="SELECT * FROM produits WHERE catp='patisserie' and qtep<>0 and (statutprod is null or statutprod<>'supprime')  ";
...
$sql="SELECT * FROM produits WHERE catp='patisserie' and qtep<>0 and (statutprod is null or statutprod<>'supprime') LIMIT ".$index_limit.','.$result_par_page;
$result=$con->query($sql);
...
for ($page=1;$page<=$nombre_page;$page++){
  echo '<a href=patisserie_client.php?page='.$page.'>'.$page.'</a> ';
}
?>
</div>
